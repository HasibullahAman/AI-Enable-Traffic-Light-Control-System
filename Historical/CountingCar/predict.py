...
# ---------------------------- import model
model = YOLO('../../../Historical/Model/best.pt')
...
def update_csv(car_count,bike_count,bus_count,truck_count):
    ...
    df.to_csv('../DataGatharing/counting_data.csv', index=False)
...
# ----------------------------------- Read the class file
my_file = open("../../../Historical/Model/coco.txt", "r")
data = my_file.read()
...
while True:
    ...
    # used to extract the bounding box coordinates of detected objects from the results object obtained from the YOLO model prediction
    a = results[0].boxes.data # in box store(with,height,left
    ...
    for index, row in px.iterrows():
        ...
        x2 = int(row[2])
        y2 = int(row[3])
        d = int(row[5]) # this will return the class index
        c = class_list[d] # here we call for class name
    ...
    if time_diff.total_seconds() >= 30:  # 15 minutes = 900 seconds
